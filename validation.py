...
class ValidationSet:
    ...
    def load(self, dataset_dir):
        ...
        for fname in fnames:
            try:
                ...
                reshaped = (reshaped - reshaped.min()) / (reshaped.max() - reshaped.min()) - 0.5
    ...
    def evaluate(self, net, iteration, noise_func):
        ...
        for idx in range(len(self.images)):
            ...
            util.save_image(self.submit_config, pred255, "img_{0}_val_{1}_pred.npy".format(iteration, idx))

            if iteration == 0:
                util.save_image(self.submit_config, orig_img, "img_{0}_val_{1}_orig.npy".format(iteration, idx))
                util.save_image(self.submit_config, noisy_img, "img_{0}_val_{1}_noisy.npy".format(iteration, idx))
